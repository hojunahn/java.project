...
// 입력 : 26 => 2 + 6 = 8 => 6 + 8 = 14 => 8 + 4 = 12 => 4 + 2 = 6 => 26
public class AddCycleEx {
    public static void main(String[] args) {
        Scanner sc = new Scanner(System.in);
        int sumNumber = 0; // 입력 받은 수의 10자리와 1의 자리를 분리해서 더하기 위한 변수
        int newNumber = 0; // 새롭게 만들어진 변수
        int cnt = 0; // 누적 횟수 구하기
        System.out.print("입력: ");
        int input = sc.nextInt();
        int tmp = input; // 입력받은 값이 변경되는 걸 막기위해서 변수에 담음
        while (true){
            sumNumber = (tmp / 10) + (tmp % 10); // 입력 받은 수의 10자리와 의 자리를 분리해서 더함
