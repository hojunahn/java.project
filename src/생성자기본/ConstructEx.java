...
// 사용용도 : 인스턴스 필드를 초기화 목적으로 사용 됨
// 생성자는 클래스 이름과 동일해야 함
// 생성자는 반환타입이 없지만 void를 사용하지 않음
// 하나의 클래스는 여러개의 생성자를 가질수 있음(생성자 오버로딩)
public class ConstructEx {
    public static void main(String[] args) {
        Car car1 = new Car(); // Car 콜래스로 객체를 생성하고 car 참조변수의 대입 함, 이때 기본 생성자 호출
...
class Car{
    ...
    Car(String com, String model){
        company = com;
        this.model = model; // 인스턴스 필드와 전달받은 매개변수의 이름이 같은 경우는 인스턴스 필드 앞에 숨겨진 this를 사용해야 함
    }
